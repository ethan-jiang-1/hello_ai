...
class SvmSize(object):
    ...
    def __init__(self):
        ...
        self.num_traning_sample = 400
        self.classifier_new = None
    ...
    def training(self):
        ...
        self.classifier_new = train(self.data,
                                    self.digits,
                                    self.num_traning_sample)

        return True

    def predict(self):

        if self.classifier_new:

            # Now predict the value of the digit on the second half:
            self.predicted = predict_result(self.data,
                                            self.digits,
                                            self.classifier_new,
                                            self.num_traning_sample)
    ...
    def plt_show(self):
        ...
        images_and_predictions = list(zip(self.digits.images[self.n_samples / 2:], self.predicted))
...
# Run if we run the svm_size.py directly
